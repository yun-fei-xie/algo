...
type Skiplist struct {
	head  *SkiplistNode // 指向最高层的第一个元素的指针
	level int           // skiplist拥有的层数 初始为0
}
...
func (Skiplist) randomLevel() int {
	lv := 1
	for lv < maxLevel && rand.Float64() < pFactor { // 4分之一
		lv++
	}
	return lv
}
...
func (s *Skiplist) Search(target int) bool {
	...
	curr = curr.forward[0] // 一直查到底 其实，在中间就可能碰到这个元素
	// 检测当前元素的值是否等于 target
